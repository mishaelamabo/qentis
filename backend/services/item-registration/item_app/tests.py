...
class TestItemModel(TestCase):
    ...
    def test_certificate_detail_cascade_delete(self):
        CertificateDetail.objects.create(
            item=self.item, student_name='John Doe', matricule='MAT001',
            degree='BSc Computer Science', institution_name='ICT University',
    ...
    def test_certificate_get_hash_fields(self):
        detail = CertificateDetail.objects.create(
            item=self.item, student_name='John Doe', matricule='MAT001',
            degree='BSc', institution_name='ICT University',
            graduation_date='2024-06-15', grade='First Class',
        )
    ...
    def test_pharmaceutical_get_hash_fields(self):
        ...
        detail = PharmaceuticalDetail.objects.create(
            item=item, drug_name='Paracetamol', batch_number='BATCH001',
            manufacturer='PharmaLab', production_date='2024-01-01',
            expiry_date='2026-01-01', factory_location='Yaounde',
        )
        h = detail.get_hash_fields()
    ...
    def test_document_get_hash_fields(self):
        ...
        detail = DocumentDetail.objects.create(
            item=item, document_type='Passport', owner_name='Jane Smith',
            issuing_authority='Ministry', reference_number='REF001',
            location='Yaounde', issue_date='2024-01-15',
        )
        h = detail.get_hash_fields()
    ...
    def test_banknote_get_hash_fields(self):
        ...
        detail = BanknoteDetail.objects.create(
            ...
            serial_number='BN001', series='2024',
...
class TestDetailStrRepresentations(TestCase):
    ...
    def _make_item(self, category):
        return Item.objects.create(
            issuer_id=uuid.uuid4(), institution_id=uuid.uuid4(), category=category,
        )
...
class TestRegisterItemEndpoint(TestCase):
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_issuer_can_register_certificate(self, mock_verify):
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_register_item_status_is_pending(self, mock_verify):
        mock_verify.return_value = make_issuer(self.issuer_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
        response = self.client.post('/api/items/register/', data=self.certificate_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_register_creates_certificate_detail(self, mock_verify):
        mock_verify.return_value = make_issuer(self.issuer_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
        response = self.client.post('/api/items/register/', data=self.certificate_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(CertificateDetail.objects.filter(item_id=response.data['item']['id']).exists())

    @patch('item_app.auth_helper.verify_token')
    def test_register_pharmaceutical(self, mock_verify):
        mock_verify.return_value = make_issuer(self.issuer_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_register_document(self, mock_verify):
        mock_verify.return_value = make_issuer(self.issuer_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
        response = self.client.post('/api/items/register/', data={
            'category': 'DOCUMENT',
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_register_banknote(self, mock_verify):
        mock_verify.return_value = make_issuer(self.issuer_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
        response = self.client.post('/api/items/register/', data={
            'category': 'BANKNOTE',
    ...
    def test_register_fails_without_token(self):
        response = self.client.post('/api/items/register/', data=self.certificate_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    @patch('item_app.auth_helper.verify_token')
    def test_register_fails_for_non_issuer(self, mock_verify):
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_register_fails_with_missing_required_fields(self, mock_verify):
        mock_verify.return_value = make_issuer(self.issuer_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
        response = self.client.post('/api/items/register/', data={
            'category': 'CERTIFICATE', 'student_name': 'John Doe',
        }, format='json')
...
class TestMyItemsEndpoint(TestCase):
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_issuer_can_view_own_items(self, mock_verify):
        mock_verify.return_value = make_issuer(self.issuer_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_returns_empty_list_for_issuer_with_no_items(self, mock_verify):
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_only_returns_own_items(self, mock_verify):
        Item.objects.create(
            issuer_id=uuid.uuid4(), institution_id=uuid.uuid4(),
            category=Item.Category.PHARMACEUTICAL,
        )
...
class TestItemDetailEndpoint(TestCase):
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_issuer_can_view_own_item(self, mock_verify):
        mock_verify.return_value = make_issuer(self.issuer_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_admin_can_view_any_item(self, mock_verify):
        mock_verify.return_value = make_admin()
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
        response = self.client.get(f'/api/items/{self.item.id}/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_other_issuer_cannot_view_item(self, mock_verify):
        mock_verify.return_value = make_issuer()
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
        response = self.client.get(f'/api/items/{self.item.id}/')
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_nonexistent_item_returns_404(self, mock_verify):
        mock_verify.return_value = make_issuer(self.issuer_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
        response = self.client.get(f'/api/items/{uuid.uuid4()}/')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_unauthenticated_can_view_item(self):
        response = self.client.get(f'/api/items/{self.item.id}/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class TestRevokeItemEndpoint(TestCase):
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_issuer_can_revoke_own_item(self, mock_verify):
        mock_verify.return_value = make_issuer(self.issuer_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_revoke_sets_revoked_at_timestamp(self, mock_verify):
        mock_verify.return_value = make_issuer(self.issuer_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_revoke_fails_with_short_reason(self, mock_verify):
        mock_verify.return_value = make_issuer(self.issuer_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
        response = self.client.put(f'/api/items/{self.item.id}/revoke/', data={
            'reason': 'Short',
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_cannot_revoke_pending_item(self, mock_verify):
        pending = Item.objects.create(
            issuer_id=uuid.UUID(self.issuer_id),
            institution_id=uuid.uuid4(),
            category=Item.Category.CERTIFICATE,
        ...
        mock_verify.return_value = make_issuer(self.issuer_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
        response = self.client.put(f'/api/items/{pending.id}/revoke/', data={
            'reason': 'Trying to revoke a pending item should fail here.',
        }, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    @patch('item_app.auth_helper.verify_token')
    def test_other_issuer_cannot_revoke_item(self, mock_verify):
        mock_verify.return_value = make_issuer()
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
        response = self.client.put(f'/api/items/{self.item.id}/revoke/', data={
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_revoke_nonexistent_item_returns_404(self, mock_verify):
        mock_verify.return_value = make_issuer(self.issuer_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
    ...
    def test_revoke_fails_without_token(self):
        response = self.client.put(f'/api/items/{self.item.id}/revoke/', data={
            'reason': 'Testing missing token scenario here.',
        }, format='json')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
...
class TestAllItemsEndpoint(TestCase):
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_admin_can_view_all_items(self, mock_verify):
        mock_verify.return_value = make_admin()
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_non_admin_cannot_view_all_items(self, mock_verify):
        mock_verify.return_value = make_issuer()
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
        response = self.client.get('/api/items/all/')
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_admin_can_filter_by_category(self, mock_verify):
        mock_verify.return_value = make_admin()
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_admin_can_filter_by_status(self, mock_verify):
        mock_verify.return_value = make_admin()
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
...
class TestPendingItemsEndpoint(TestCase):
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_admin_can_view_pending_items(self, mock_verify):
        mock_verify.return_value = make_admin()
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_pending_only_returns_pending_items(self, mock_verify):
        mock_verify.return_value = make_admin()
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
        response = self.client.get('/api/items/pending/')
...
class TestApproveItemEndpoint(TestCase):
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_admin_can_approve_item(self, mock_verify):
        mock_verify.return_value = make_admin(self.admin_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_cannot_approve_already_registered_item(self, mock_verify):
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_approve_nonexistent_item_returns_404(self, mock_verify):
        mock_verify.return_value = make_admin(self.admin_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
...
class TestRejectItemEndpoint(TestCase):
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_admin_can_reject_item(self, mock_verify):
        mock_verify.return_value = make_admin(self.admin_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_reject_nonexistent_item_returns_404(self, mock_verify):
        mock_verify.return_value = make_admin(self.admin_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
    ...
    @patch('item_app.auth_helper.verify_token')
    def test_cannot_reject_already_registered_item(self, mock_verify):
        self.item.status = Item.Status.REGISTERED
        self.item.save()
        mock_verify.return_value = make_admin(self.admin_id)
        self.client.credentials(HTTP_AUTHORIZATION='Bearer faketoken')
...
class TestViewHelperFunctions(TestCase):
    ...
    def test_call_blockchain_service_returns_mock_on_failure(self):
    ...
    def test_call_output_service_returns_mock_on_failure(self):
